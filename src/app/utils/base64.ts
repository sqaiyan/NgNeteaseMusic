 ...
 */
const str2binl = (str: String): Array<any> => {
  const bin = Array();
 ...
 */
const binl2b64 = (binarray): String => {
  const tab =
  ...
  for (let i = 0; i < binarray.length * 4; i += 3) {
    ...
    for (let j = 0; j < 4; j++) {
      if (i * 8 + j * 6 > binarray.length * 32) {
        ...
        str += tab.charAt((tripconst >> (6 * (3 - j))) & 0x3f);
      }
    }
  }
  return str;
...
const str2Arr = (str: String): Array<any> => {
  const bytes = [];
...
const id2Url = (pic_str: String): String => {
  pic_str += '';
  const magic = str2Arr('3go8&$8*3*3h0k(2)2');
  const songId = str2Arr(pic_str);
  for (let i = 0; i < songId.length; i++) {
    songId[i] = songId[i] ^ magic[i % magic.length];
  }
  const songIds = arr2Str(songId);
  let res = b64_md5(songIds);
  res = res.replace(/\//g, '_');
  res = res.replace(/\+/g, '-');
  res += '==';
