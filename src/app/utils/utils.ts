...
function time(date) {
  ...
  const Qq =
    jm.getHours() * 36e5 + jm.getMinutes() * 6e4 + jm.getSeconds() * 1e3;
  if (day === jm.getDate()) {
    if (Fo < 36e5) {
...
function parse_lrc(lrc_content) {
  const now_lrc = [];
  const lrc_row = lrc_content.split('\n');
  let scroll = true;
  for (const i in lrc_row) {
    if (lrc_row[i].indexOf(']') === -1 && lrc_row[i]) {
      ...
      let tmp2 = tmp[0].substr(1, 8);
      tmp2 = tmp2.split(':');
      const lrc_sec = parseInt(tmp2[0] * 60 + tmp2[1] * 1 + '', 10);
...
export { removeNgTag, time, parse_lrc, emoji, deepclone };
